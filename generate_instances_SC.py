...
#            lp_file.write(f"C{count+1}:" + "".join([f" + x{node+1}" for node in sorted(group)]) + " <= 1\n")
            lp_file.write("C{}:".format(count+1) + "".join([" + x{}".format(node+1) for node in sorted(group)]) + " <= 1\n")
...
def generate_capacited_facility_location(random, filename, n_customers, n_facilities, ratio):
    ...
    with open(filename, 'w') as file:
        file.write("minimize\nobj:")
        file.write("".join([" +{trans_costs[i, j]} x_{}_{}".format(i+1,j+1) for i in range(n_customers) for j in range(n_facilities)]))
        file.write("".join([" +{fixed_costs[j]} y_{}".format(j+1) for j in range(n_facilities)]))

        file.write("\n\nsubject to\n")
        for i in range(n_customers):
            file.write("demand_{}:".format(i+1) + "".join([" -1 x_{}_{}".format(i+1,j+1) for j in range(n_facilities)]) + " <= -1\n")
        for j in range(n_facilities):
            file.write("capacity_{j+1}:".format(j+1) + "".join([" +{} x_{}_{}".format(demands[i],i+1,j+1) for i in range(n_customers)]) + " -{} y_{} <= 0\n".format(capacities[j],j+1))

        # optional constraints for LP relaxation tightening
        file.write("total_capacity:" + "".join([" -{} y_{}".format(capacities[j],j+1) for j in range(n_facilities)]) + " <= -{}\n".format(total_demand))
        for i in range(n_customers):
            for j in range(n_facilities):
                file.write("affectation_{}_{}: +1 x_{}_{} -1 y_{} <= 0".format(i+1,j+1,i+1,j+1,j+1))
        ...
        file.write("\nbinary\n")
        file.write("".join([" y_{}".format(j+1) for j in range(n_facilities)]))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--problem',
        help='MILP instance type to process.',type=str,
        default='setcover'
    ...
    if args.problem == 'setcover':
        ...
        #n = 1000
        n=300
        lp_dir = 'instance/SC/train'
        print("{} instance in {}".format(n,lp_dir))
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, 'instance_{}.lp'.format(i+1)) for i in range(n)])
        ...
        #n = 1000
        n = 10
        lp_dir = 'data/instances/setcover/valid_{}r_{}c_{}d'.format(nrows,ncols,dens)
        print("{} instances in {}".format(n,lp_dir))
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, 'instance_{}.lp'.format(i+1)) for i in range(n)])
        nrowss.extend([nrows] * n)
        ncolss.extend([ncols] * n)
        ...
        n = 100
        nrows = 500
        lp_dir = 'data/instances/setcover/transfer_{}r_{}c_{}d'.format(nrows,ncols,dens)
        print("{} instances in {}".format(n,lp_dir))
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, 'instance_{}.lp'.format(i+1)) for i in range(n)])
        nrowss.extend([nrows] * n)
        ...
        nrows = 500
        ncols = 1000
        lp_dir = 'data/instances/setcover/test_{}r_{}c_{}d'.format(nrows,ncols,dens)
        print("{} instances in {}".format(n,lp_dir))
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, 'instance_{}.lp'.format(i+1)) for i in range(n)])
        nrowss.extend([nrows] * n)
